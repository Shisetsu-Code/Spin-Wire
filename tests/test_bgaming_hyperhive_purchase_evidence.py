...
def test_known_purchase_literal_without_req_scope_is_discovery_only() -> None:
    ...
    try:
        ...
        modes = hyperhive.discover_modes_from_bundle(runtime, timeout_s=1, bundle_text=bundle, engine_contract="")
    finally:
        runtime.session.close()
    purchase = {str(mode["id"]): mode for mode in modes}["PURCHASE_BUY_BONUS"]
...
def test_request_scoped_purchase_literal_remains_executable() -> None:
    runtime = _runtime()
    try:
        bundle = 'var request={req:{bet:stake,bet_type:"bet",purchased_feature:"buy_bonus"}};'
        modes = hyperhive.discover_modes_from_bundle(runtime, timeout_s=1, bundle_text=bundle, engine_contract="")
    finally:
        runtime.session.close()
    purchase = {str(mode["id"]): mode for mode in modes}["PURCHASE_BUY_BONUS"]
...
def test_contract_evidence_reports_structure_without_source_literals() -> None:
    text = ('const secretValue="do-not-export";'
    ...
    assert "do-not-export" not in repr(evidence)
    assert "secretValue" not in repr(evidence)


def test_contract_evidence_traces_identifier_passed_as_params_req() -> None:
    text = ('const r={};r.bet=stake;r.bet_type="bet";'
...
def test_contract_evidence_traces_req_alias_object_bet_fields() -> None:
    text = ('const r={bet:stake,bet_type:"bet"};'
            'const request={method:"play",params:{token:t,req:r}};')
    shorthand = ('const bet=stake;const r={bet};'
                 'const request={method:"play",params:{token:t,req:r}};')
    assert hyperhive_wire._wire_contract_evidence(text)["req_alias_object_bet_value"] is True
    assert hyperhive_wire._wire_contract_evidence(shorthand)["req_alias_object_bet_shorthand"] is True


def test_production_req_bet_evidence_accepts_only_linked_alias_object() -> None:
    linked = ('const r={bet:stake,bet_type:"bet"};'
              'const request={method:"play",params:{token:t,req:r}};')
    unrelated = ('const r={bet:stake};const q={bet_type:"bet"};'
...
def test_req_bet_type_ignores_unrelated_assignment_when_play_alias_is_freebet_only() -> None:
    client = (
        'async play(t,e,i,s){const n={bet:t,purchased_feature:e,buy_id:i};'
        's&&(n.bet_type="freebet");'
        'return JSON.stringify({method:"play",params:{token:this._token,req:n,state_lock:this._stateLock}})};'
        'other.req.bet_type="bet";'
    )
    assert hyperhive_wire._has_req_bet_type_evidence(client) is False


def test_req_bet_type_accepts_value_on_same_play_alias() -> None:
    client = (
        'async play(t){const n={bet:t,bet_type:"default"};'
        'return JSON.stringify({method:"play",params:{token:this._token,req:n,state_lock:this._stateLock}})}'
    )
    assert hyperhive_wire._has_req_bet_type_evidence(client) is True


def test_mode_diagnostic_metadata_preserves_source_free_contract_evidence() -> None:
    mode = {
        "id": "SPIN", "kind": "SPIN", "request": {"bet_type": "bet"},
        "source": "live-client-evidence-incomplete", "executable": False,
        "discovery_state": "CONTRACT_UNRESOLVED",
        "contract_evidence": {"method_play": True, "req_bet_shorthand": True,
                              "bet_token_count": 4,
